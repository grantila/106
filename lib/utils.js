...
exports.timestamp = function( date, opts )
{
	date = date === undefined ? new Date( ) : date;
	opts = opts === undefined ? {} : opts;

	var withoutMilliseconds = opts.withoutMilliseconds || false;

	var reStripTZ = /Z.*/;
	var reStripMillisAndTZ = /\.\d+Z.*/;
	var reStrip = withoutMilliseconds ? reStripMillisAndTZ : reStripTZ;

	return date.toJSON( ).replace( /T/, ' ' ).replace( reStrip, '' );
};

exports.prettyPrintJson = function( object, opts )
{
	opts = opts === undefined ? {} : opts;
		...
		var recurse = function( object, level )
		{
			...
			else if ( object.constructor === Array )
			{
				...
				for ( var i = 0; i < object.length; ++i )
				{
					...
					s += keyIndents + recurse( object[ i ], level + 1 ) + comma + "\n";
				}
			...
			else if ( object.constructor === Object )
			{
				...
				for ( var key in object )
				{
					if ( object.hasOwnProperty( key ) )
						entries.push( keyIndents + colors.fg.green( printKey( key ) ) + ": " + recurse( object[ key ], level + 1 ) );
				}
				if ( entries.length > 0 )
				{
					s += entries.join( ",\n" ) + "\n";
...
exports.parseLog = function( level, msg, meta, opts )
{
	opts = opts === undefined ? {} : opts;

	var internal = meta.internal;
	...
	if ( opts.prettyPrint )
	{
		...
		var printSequencePrefix = function( )
		{
			...
			{
				var symbol = internal.sequenceDirection.getSymbol( );
				var direction = internal.sequenceDirection.getDirection( );

				if ( direction === 'IN' )
					internal.sequence.begin( internal );
				else
		...
		if ( opts.colorized )
		{
			line = internal.timestamp + " " + colors.colorizeLevel( level ) + " - " + colors.colorizePrefix( internal.prefix ) + ": ";
		}
		else
		{
			line = internal.timestamp + " " + level + " - " + internal.prefix + ": ";
		}
		...
		if ( opts.printMeta === undefined || opts.printMeta )
		{
			if ( meta != null )
			{
				line += "\n" + exports.prettyPrintJson( meta, opts );
			}

			if ( error != null )
			{
				line += "\n" + exports.prettyPrintJson( error, opts );
			}
		}

		out.internal.line = line;
	}
